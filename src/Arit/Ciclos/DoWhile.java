...
public class DoWhile extends Instruccion {
    ...
    public DoWhile(Expresion condicion, LinkedList<NodoAst> sentencias, int fila, int columna) {
        super(fila, columna);
    ...
    public Object ejecutar(Entorno en) {
        try {
            ...
            if (condicion_while instanceof Vector) {
                ...
                Informacion.agregarError(new ErrorAr("Semantico", "La expresion del Do_While no es booleana ", fila, columna));
            }
        } catch (Exception e) {
            Informacion.agregarError(new ErrorAr("Semantico", "La expresion del Do_While no es booleana ", fila, columna));
        }
    ...
    public NodoDot generarDot(int numero) {
        ...
        String cuerpo = nodo + "[label=\"Sentencia_Do_While\"];\n";
