...
public class Ncol extends Funcion {
    ...
    public Ncol(String identificador, LinkedList<Parametro> parametros, LinkedList<NodoAst> sentencias, int fila, int columna) {
    ...
    public Object ejecutar(Entorno en) {
        try {
            Simbolo sim = en.getSimbolo("parametro&&expresion&&ncol01210");
            Object val = sim.getValor();
            if (val instanceof Matris) {
                Matris mat = (Matris) val;
                int col = mat.getColumna();
                Vector nuevo = new Vector();
                nuevo.agregarFinal(new Nodo(col));
                nuevo.ponerTipoNuevo();
                return nuevo;
            } else {
                Informacion.Informacion.agregarError(new ErrorAr("Semantico", "La expresion en la funcion nCol no es una matris", this.fila, this.columna));
            }
        } catch (Exception e) {
